...
st.markdown(
    ...
    .st-key-search_hover .hover-title {
        font-size: 3.2rem;
        line-height: 0.92;
        font-weight: 950;
        letter-spacing: -2px;
    ...
    .st-key-media_hover .stButton,
    .st-key-media_hover [data-testid="stImage"],
    .st-key-camera_hover [data-testid="stCameraInput"],
    .st-key-camera_hover .stButton,
    .st-key-camera_hover [data-testid="stImage"] {
        opacity: 0;
    ...
    .st-key-media_hover:hover .stButton,
    .st-key-media_hover:hover [data-testid="stImage"],
    .st-key-camera_hover:hover [data-testid="stCameraInput"],
    .st-key-camera_hover:hover .stButton,
    .st-key-camera_hover:hover [data-testid="stImage"] {
        opacity: 1;
        transform: translateY(0);
        pointer-events: auto;
    }
    ...
    .st-key-latest_selector .preview-window,
    .st-key-oldest_selector .preview-window,
    ...
    .st-key-latest_selector:hover .preview-window,
    .st-key-oldest_selector:hover .preview-window,
...
def save_item(label, location, found_date, image):
    ...
    connection.execute(
        """
        INSERT INTO items
        (label, location, found_date, image_path, created_at, found)
        VALUES (?, ?, ?, ?, ?, 0)
        """,
        (
            label,
...
def mark_item_as_found(item_id):
    ...
    connection.execute(
        "UPDATE items SET found = 1 WHERE id = ?",
        (item_id,),
    )
...
def get_oldest_items(limit=20):
    ...
    rows = connection.execute(
        ...
        ORDER BY
            CASE
                WHEN found_date IS NULL OR found_date = '' THEN 1
                ELSE 0
            END,
            date(found_date) ASC,
...
def search_items(query="", loss_date_text=""):
    ...
    if search_date:
        def date_difference(row):
            stored_date = parse_date(row["found_date"])

            if stored_date is None:
                return 999999999

            return abs((stored_date - search_date).days)

        rows = sorted(rows, key=date_difference)
                    ...
                    if image_path and os.path.exists(image_path):
                        ...
                        st.markdown(
                            ...
                            <div class="result-info">
                                Fundort: {item["location"] or "Nicht angegeben"}
                            </div>

                            <div class="result-info">
                                Gefunden am: {item["found_date"] or "Nicht angegeben"}
                            </div>
                            """,
                            unsafe_allow_html=True,
                        )
            ...
            if uploaded_file is not None:
                ...
                if save_upload:
                    if not upload_location.strip():
                        st.warning("Bitte gib einen Fundort ein.")

                    elif not upload_date.strip():
                        st.warning("Bitte gib ein Funddatum ein.")
...
# =========================================================
# HILFSFUNKTION FÜR BILD-VORSCHAU
# =========================================================
...
def create_preview_html(items):
    ...
    # Bilder zweimal einsetzen, damit die Animation
    # nahtloser wirkt.
...
# =========================================================
    ...
    if st.session_state.fundstuecke_page == "selection":
        ...
        with col1:
            ...
            with st.container(key="latest_selector"):
                ...
                st.markdown(
                    create_preview_html(latest_items),
                    unsafe_allow_html=True,
                )
                ...
                if st.button(
                    ...
                    st.session_state.fundstuecke_page = "latest"
                    st.rerun()
        ...
        with col2:
            ...
            with st.container(key="oldest_selector"):
                ...
                st.markdown(
                    create_preview_html(oldest_items),
                    unsafe_allow_html=True,
                )
                ...
                if st.button(
                    ...
                    st.session_state.fundstuecke_page = "oldest"
                    st.rerun()
...
if page == "Fundstücke" and st.session_state.fundstuecke_page in [
    "latest",
    "oldest",
]:
